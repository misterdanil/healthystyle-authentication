...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	Page<User> findByNameAndBirthYearAndSexAndRegionId(String name, Integer birthYear, Type sex, Long regionId, Pageable pageable);
...
//	Page<User> findLastCreated(String name, Pageable pageable);
	...
	//service findBYUsernameAndName with sort
	//	@Query("SELECT u FROM User u WHERE LOWER(CONCAT(u.firstName, ' ', u.lastName)) LIKE LOWER(%:name%) OR LOWER(u.username) LIKE LOWER(%:name%) ORDER BY u.removedOn DESC")
//	Page<User> findRemoved(String name, Pageable pageable);
	
	@Query("SELECT EXISTS (SELECT ur FROM User u INNER JOIN u.roles ur WHERE u = :user AND ur = :role)")
